...
-------------
Na pacata vila campestre de Ponteironuloville, todos os telefones têm 6 dígitos.
...
def duplo(number):
    '''
    Função que verifica se dois digitos consecutivos são iguais.
    :param number: numero que deseja-se verificar igualdade dos digitos consecutivos.
    :return: booleano, falso se dois digitos consecutivos forem iguais e true se nao forem.
    '''
    n = str(number)
    i = k = 0
    while i < len(n) - 1:
...
def somapar(number):
    '''
    Função que verifica a soma dos dígitos do número é par.
    :param number: numero que deseja-se somar os digitos.
    :return: booleano, true se a soma for par e falso se for impar.
    '''
    soma = 0
    n = str(number)
    for i in n:
...
s = 0
for n in lista:
    if ((duplo(n)) and (somapar(n)) and (n[0] != n[-1])):
        s += 1
